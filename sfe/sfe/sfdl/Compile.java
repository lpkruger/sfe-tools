...
import sfe.sfdl.CompilerOutput.FunctionOutput;
import sfe.sfdl.SFDL.CompoundType;
...
public class Compile {
	...
	GateBase[] assignToVar(SFDL.LValExpr lval, GateBase[] cc) {
	...
	GateBase[] retrieveFromVar(SFDL.LValExpr lval) {
		String id = lval.uniqueStr();
	...
	CompilerOutput compileExpr(SFDL.Expr ex) {
		return ex.compile(this);
	}

	CompilerOutput.FunctionOutput compileFunction(SFDL.FunctionDef fun) {
		openScope();
		...
		return new CompilerOutput.FunctionOutput(circ);

	}
	
	CompilerOutput compileBlock(SFDL.Block block) {
		for (Expr expr : block.stmts) {
			compileExpr(expr);
		}
		return new CompilerOutput(new GateBase[0]);
	}
	
	CompilerOutput compileAssignExpr(SFDL.AssignExpr expr) {
		...
		return new CompilerOutput(cc);
	}

	CompilerOutput compileAddExpr(SFDL.AddExpr expr) {
		...
		SFDL.IntType type = castToInt(expr.type);
	...
	static class CompiledFunction {
		SFDL.FunctionDef fun;
		Circuit cc;
	}
	...
	CompilerOutput compileIntConst(IntConst intConst) {
		...
		if (n==0) {
			return new CompilerOutput(FALSE_GATE);
		}
	...
	CompilerOutput compileSubExpr(SubExpr expr) {
		throw new CompilerError("not yet implemented: subExpr");
	}
	CompilerOutput compileXorExpr(XorExpr expr) {
		GateBase[] lc = compileExpr(expr.left).cc;
		GateBase[] rc = compileExpr(expr.right).cc;
	...
	CompilerOutput compileEqExpr(EqExpr expr) {
		GateBase[] lc = compileExpr(expr.left).cc;
		GateBase[] rc = compileExpr(expr.right).cc;
		...
		return new CompilerOutput(cc[cc.length-1]);
	}
	...
	public CompilerOutput compileLessThanExpr(LessThanExpr expr) {
		...
		return new CompilerOutput(cc[0]);
	...
	public CompilerOutput compileIfExpr(IfExpr ife) {
		CompilerOutput out = compileExpr(ife.cond);
		...
		return new CompilerOutput(new Gate[0]);
	}
