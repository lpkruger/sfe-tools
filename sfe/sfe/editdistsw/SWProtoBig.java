...
public class SWProtoBig {
	...
	public static class Alice {
		...
		void go() throws Exception {
			...
			Circuit circuit = CircuitParser.readFile("sw_" + N_BITS + "_" + astrlen + "_" + bstrlen + ".circ");
			VarDesc bdv = VarDesc.readFile("sw_" + N_BITS + "_" + astrlen + "_" + bstrlen + ".split");
			...
			int cBits=CHAR_BITS;
			//int aStart=CHAR_BITS*bstrlen;
			int aStart=0;
			//for (int i=str1.length()-1; i>=0; --i) {
			//	byte c0 = (byte) str1.charAt(i);
			//	mapBits(BigInteger.valueOf(c0), vals, aStart, (aStart+=cBits)-1);
			//}
			SmithW sw = new SmithW();
			sw.populateDB();
			boolean[] alicebits = sw.str2bits(str1);
			for (int i=0; i<alicebits.length; ++i) {
				vals.put(aStart+i, alicebits[i]);
			}
	...
	public static class Bob {
		...
		void go() throws Exception {
			...
			int cBits=CHAR_BITS;
			//int bStart=0;
			int bStart=CHAR_BITS*astrlen;
			
			SmithW sw = new SmithW();
			sw.populateDB();
			boolean[] bobbits = sw.str2bits(str2);
			for (int i=0; i<bobbits.length; ++i) {
				vals.put(bStart+i, bobbits[i]);
			}
