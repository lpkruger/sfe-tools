...
public class SWProtoBlock {
	
	static void D(Object s) {
		System.out.println(s);
	}
	
	static SmithW smw = new SmithW();
	static {
		try {
			smw.populateDB();
	...
	public static class Alice {
		...
		void go() throws Exception {
			...
			for (int i=1; i<=astrlen; ++i) {
				// for S-W set to 0 not i
				matrix[i][0] = 0;
			}
			for (int j=1; j<=bstrlen; ++j) {
				// for S-W set to 0 not j
				matrix[0][j] = 0;
			}
			...
			//circuit = CircuitParser.readFile("editdist/block_" + N_BITS + "_" + BLOCK_SIZE + "_" + BLOCK_SIZE + ".circuit");
			fmt = FmtFile.readFile("swblock_" + N_BITS + "_" + BLOCK_SIZE + "_" + BLOCK_SIZE + ".fmt");
		...
		public void computeRecurrenceParallel(int[][] pairs) throws Exception {
			...
			for (int z=0; z<pairs.length; ++z) {
				...
				ccs[z] = CircuitParser.readFile("swblock_" + N_BITS + "_" + BLOCK_SIZE + "_" + BLOCK_SIZE + ".circ");
				int ibase = pairs[z][0];
				int jbase = pairs[z][1];

				for (int i=0; i<=BLOCK_SIZE; ++i) {
					if (i<BLOCK_SIZE) {
						char c0 = str1.charAt(ibase + i);
						fmt.mapBits(smw.char2num(c0), vals[z], "input.alice.x[" + i + "]");
					}
		...
		public void computeRecurrence(int ibase, int jbase) throws Exception {
			...
			Circuit circuit = CircuitParser.readFile("swblock_" + N_BITS + "_" + BLOCK_SIZE + "_" + BLOCK_SIZE + ".circ");
			TreeMap<Integer,Boolean> vals = new TreeMap<Integer,Boolean>();
			
			for (int i=0; i<=BLOCK_SIZE; ++i) {
				if (i<BLOCK_SIZE) {
					char c0 = str1.charAt(ibase + i);
					fmt.mapBits(smw.char2num(c0), vals, "input.alice.x[" + i + "]");
				}
	...
	public static class Bob {
		...
		void computeRecurrenceParallel(int[][] pairs) throws Exception {
			...
			for (int z=0; z<pairs.length; ++z) {
				...
				for (int i=0; i<=BLOCK_SIZE; ++i) {
					if (i<BLOCK_SIZE) {
						char c0 = str2.charAt(jbase + i);
						fmt.mapBits(smw.char2num(c0), vals[z], "input.bob.x[" + i + "]");
					}
		...
		void computeRecurrence(int ibase, int jbase) throws Exception {
			...
			for (int i=0; i<=BLOCK_SIZE; ++i) {
				if (i<BLOCK_SIZE) {
					char c0 = str2.charAt(jbase + i);
					fmt.mapBits(smw.char2num(c0), vals, "input.bob.x[" + i + "]");
				}
