...
public class BDDCrypt {
	...
	byte[] createLabel() {
		return keyToBytes(KG.generateKey());
	}
	
	// encrypt a link according to BDD encryption algorithm
	byte[] encLink(SecretKey sv1, SecretKey sl, SecretKey sv2, byte[] lv2) {
		SecretKey esk = xor(sv1, sl);
		byte[] sv2bytes = keyToBytes(sv2);
	...
	byte[] keyToBytes(SecretKey k) {
		return k.getEncoded();
	}
	
	// helper conversion function	
	SecretKey bytesToKey(byte[] k) {
