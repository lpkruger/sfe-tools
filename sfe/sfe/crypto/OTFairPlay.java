...
import fairplay.BOAL.MyUtil;
import fairplay.BOAL.OTr.OT;
import fairplay.BOAL.OTr.OTMESS;
import fairplay.BOAL.OTr.OTTASK;
 ...
 * @author lpkruger
 *
 * wrapper around FairPlay's OT code
 */
public class OTFairPlay {
	// fairplay doesn't preserve length.
	// also fairplay does weird and annoying things
	// when the first byte is 0.
	...
	public static class Chooser {
		...
		public BigInteger[] go() throws Exception {
		    ...
		    /*
		    System.out.println("OTs " + v.size());
		    ByteCountOutputStream testb;
		    ObjectOutputStream test = new ObjectOutputStream(testb = new ByteCountOutputStream(
		    		new FileOutputStream("/dev/null")));
		    test.writeObject(v);
		    test.close();
		    System.out.println("OTs bytes: " + testb.cnt);
	        */
