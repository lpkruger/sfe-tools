...
public class ESLCFile {
	...
	public ESLCFile(ESL esl, PrintStream ps) {
		this.ps = ps;
		this.esl = esl;
		this.edl = esl.edl;
	...
	void emitVarDecls() {
		...
		while (it.hasNext()) {
			ESL.VarDecl var = (ESL.VarDecl) it.next();
			String initval = "";
	...
	void emitMatch(String evname) {
		...
		int count = 0;
		while (it.hasNext()) {
			ESL.Match match = (ESL.Match) it.next();
	...
	void emitTest(EDL.Event ev, ESL.Match match, int mcount) {
		...
		if (ev.args.length > 0) {
			...
			for (int i = 0; i < ev.args.length; ++i) {
				...
				ps.println(separ);
				separ = " && ";
				if (match.argrules[i] instanceof ESL.TrueRule) {
					ps.println("1");
				} else if (match.argrules[i] instanceof ESL.FalseRule) {
					ps.println("0");
				} else if (match.argrules[i] instanceof ESL.TypeRule) {
					// algorithm: find all subtypes of type and emit tests
					
					ESL.TypeRule trule = (ESL.TypeRule) match.argrules[i];
					...
					ps.println(")");
				} else if (match.argrules[i] instanceof ESL.RegexRule) {
					String s =
						emitRegexTest(
							(ESL.RegexRule) match.argrules[i],
							ev.args[i].name);
					ps.println(s);
				} else {
			...
			ps.println(
				"     return "
					+ (match.policy == ESL.POLICY_ALLOW
						? ((mcount+1) + ";")
						: "EDL_DENY;"));
			ps.println("   }");
		}
	}
	String emitRegexTest(ESL.RegexRule rule, String argname) {
