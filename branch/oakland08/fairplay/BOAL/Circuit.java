...
public class Circuit implements Serializable {
    ...
    public Gate addGate(int n_ins, int type, boolean out_gate) {
        ...
        circuit.add(newgate);
        logger.info("add gate #" + csize + " with " + n_ins +
            " inputs");
    ...
    */
     ...
     */
	 ...
	 for (int i = 0; i < circuit.size(); i++) {
            logger.debug("isCorrect: checking gate in line number " + i);
    ...
    public void evalCircuit() {
        logger.info("evalCircuit: having " + circuit.size() + " gates");

        for (int i = 0; i < circuit.size(); i++) {
            logger.debug("evalCircuit: evaluating gate in line number " + i);
            getGate(i).evalGate();
        }
    }
    ...
    public void evalGarbledCircuit(boolean alice_interpret, boolean bob_interpret) {
        logger.info("evalGarbledCircuit: having " + circuit.size() + " gates");

        for (int i = 0; i < circuit.size(); i++) {
            logger.debug("evalGarbledCircuit: evaluating gate in line number " +
                i);
