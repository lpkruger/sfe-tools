...
public class Parser {
    ...
    private boolean parseLineNum() throws ParseError {
        ...
        logger.info("parseLineNum: line number = " + line_num);
    ...
    private int parseLineName() throws ParseError {
        ...
        if (inp_line) {
            logger.info("parseLineName: input line " + line_num);
            rc = INPUT_LINE;
        }

        if (gate_line) {
            logger.info("parseLineName: gate line " + line_num);
            rc = GATE_LINE;
        }
	...
	String sseed = args[1];
        ...
        try {
            ...
            logger.error("main: cannot open/close " + circuit_filename + " - " +
                e.getMessage());
        } catch (ParseError e) {
            logger.error("main: parsing " + circuit_filename + " failed.");
        } catch (Exception e) {
        ...
        try {
            ...
            logger.error("main: cannot open/close " + fmt_filename + " - " +
                e.getMessage());
        } catch (FormatterError e) {
            logger.error("main: parsing " + fmt_filename + " failed.");
        } catch (Exception e) {
            logger.error("main: exception - " + e.getMessage());
        }
