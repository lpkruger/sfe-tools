...
public class MyUtil {
    ...
    public static int WRITE_MODE = 0;  // 1 = circuit, 
    ...
    // special hack function because FairPlay blows so it needs a handicap to
    // compete with us.
    public static void setDeltaTime (long time) {
    	rec_time = System.currentTimeMillis() - time;
    }
    //---------------------------------------------------------------
    ...
    */
    ...
    public static void sendOTMESS(ObjectOutputStream oos, OTMESS otmess) {
	    ...
	    MyUtil.WRITE_MODE=MODE_OT;
            ...
            e.printStackTrace();
            //System.exit(-1);
            throw new RuntimeException("sendOTMESS");
        }
    }
    ...
    public static void sendVector(ObjectOutputStream oos, Vector v) {
        try {
	    throw new RuntimeException("Unused code???");
            oos.writeObject(v);
            oos.flush();
        } catch (IOException e) {
