...
public class SFECompiler {
			...
			do {
				while (tokenizer.tokenType() == Tokenizer.SYMBOL) {
					switch (tokenizer.symbol()) {
						...
						default:
							throw new ParseException("~ or - or @ or ( is expected",
							                         tokenizer.lineNumber());
					}
	                   ...
	                   throws EmptyStackException
	{
		...
		if ((operator instanceof UnaryMinusOperator) ||
				(operator instanceof UnaryParityOperator) ||
	                      ...
	                      throws ParseException, IOException
	{
		...
		switch (tokenizer.symbol()) {
			...
			case '-':
				...
				/*
			case '*':
				insertNewOperator(operators, operands, new MultOperator());

				break;
				*/
			case '/':
				insertNewOperator(operators, operands, new DivOperator());
	                    ...
	                    throws IOException
	{
		...
		try {
			...
			System.err.println("Parse error on line " + pe.getErrorOffset() + ": " +
			                   pe.getMessage());
			System.err.println(" token " + compiler.tokenizer.stringVal());
			pe.printStackTrace(System.err);
			System.exit(1);
		} catch (Exception ex) {
			System.err.println("Internal compiler error on line " + 
					compiler.tokenizer.lineNumber() + " token " + compiler.tokenizer.stringVal());
