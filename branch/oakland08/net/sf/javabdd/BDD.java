...
public abstract class BDD {
    ...
    public static void printDot(BDD[] bddz) {
    	...
    	int nCount = 0;
    	for (BDD bdd : bddz) {
    		nCount += bdd.nodeCount();
    	}
    	boolean[] visited = new boolean[nCount+2];
    	visited[0] = true; visited[1] = true;
    	HashMap map = new HashMap();
    	map.put(bddz[0].getFactory().zero(), new Integer(0));
    	map.put(bddz[0].getFactory().one(), new Integer(1));
    	
    	int current = 1;
    	for (BDD bdd : bddz) {
    		current = bdd.printdot_rec(out, current, visited, map);
    	}
